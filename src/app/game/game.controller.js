  ...
  function GameController($scope, $q, $state, $stateParams, $localStorage, cfActions) {
    ...
    if (!$stateParams.game.$bindTo) {
      ...
      $stateParams.game.$bindTo(vm, 'game').then(function() {
        ...
        function startTurn() {
          vm.game.roundLog === undefined ? vm.game.roundLog = [] : null;
        ...
        function movePlayer() {
          ...
          (vm.game.currentPlayer.position === undefined) ? vm.game.currentPlayer.position = 0: '';
          var log = vm.game.currentPlayer.name + ' würfelt eine ' + vm.game.rolled + ' und rückt von ' + vm.game.currentPlayer.position + ' auf ' + ratRace[(vm.game.currentPlayer.position + vm.game.rolled) % ratRace.length].type;
